            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Your attendance in <span className="font-medium">Data Structures</span> is at 72%. Minimum required is 75%.
                </p>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div variants={item} className="card-elevated p-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="status-badge status-verified">Paid</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">Exam Fee</span>
                <span className="status-badge status-pending">Due Dec 30</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">Library Fine</span>
                <span className="status-badge status-error">₹50 Pending</span>
              </div>
...
function ScheduleItem({ time, subject, room, status }: ScheduleItemProps) {
  const statusStyles = {
...
function JobCard({ company, role, location, deadline, matchScore }: JobCardProps) {
  return (
    <div className="p-4 rounded-lg border border-border hover:border-accent/30 transition-colors cursor-pointer">
      <div className="flex items-start justify-between">
        ...
        <div className="text-right">
          <span className="text-sm font-semibold text-accent">{matchScore}% Match</span>
        </div>
      </div>
...
function DeadlineItem({ title, course, dueDate, daysLeft }: DeadlineItemProps) {
  ...
  return (
    <div className="flex items-center gap-3">
      <div className={`w-10 h-10 rounded-lg bg-secondary flex items-center justify-center ${urgency}`}>
      ...
      <div className="text-right">
        <p className={`text-sm font-medium ${urgency}`}>{daysLeft}d left</p>
